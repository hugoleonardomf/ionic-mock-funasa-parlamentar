...
@Component({
  selector: 'page-ufEmendas',
  templateUrl: 'ufEmendas.html'
})

export class UfEmendasPage {
  ...
  getItems(ev: any) {
    ...
    if (val && val.trim() != '') {
      this.items = this.items.filter((item) => {
        return (this.utilsProvider.removeAcento(item.uf).toLowerCase().indexOf(this.utilsProvider.removeAcento(val).toLowerCase()) > -1);
      })
    }
    else {
      this.items = this.itemsStored;
